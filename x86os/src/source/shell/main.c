...
static const cli_cmd_t cmd_list[] = {
    ...
    {
        .name = "less",
        .useage = "less [-l] file -- list text file content",
        .do_func = do_less,
    },
    {
        .name = "cp",
...
static void cli_init(const char * promot, const cli_cmd_t * cmd_list, int cnt) {
    ...
    cli.cmd_start = cmd_list;      // cmd_start 指向第1个命令
    cli.cmd_end = cmd_list + cnt;  // cmd_end 指向最后1个命令的下一个命令位置
}
